...
from jnt_admin_tools.menu.utils import get_admin_menu
...
tag_func = register.inclusion_tag(
    "jnt_admin_tools/menu/dummy.html", takes_context=True
)
...
def admin_tools_render_menu(context, menu=None):
    ...
    bookmark = None
    if [c for c in menu.children if isinstance(c, items.Bookmarks)]:
...
def admin_tools_render_menu_css(context, menu=None):
    ...
    context.update(
        {
            "template": "jnt_admin_tools/menu/css.html",
